...
public class PhraseTokenizer extends Tokenizer {
    ...
    public boolean incrementToken() throws IOException {
        ...
        int[] nextPhrase = FindNextPhraseLocationAndLength();
        
		if (nextPhrase[0] == position)
        {
            nextIndex = position + nextPhrase[1];
            //isPhrase = false;
        }
        else
        {
            nextIndex = stringToTokenize.indexOf(' ', position);
        }	
		
		
		while (stringToTokenize.indexOf(' ', nextIndex + 1) == (nextIndex + 1))
		{
			nextIndex++;
		}
        ...
        if (nextIndex != -1) {
            String nextToken = this.stringToTokenize.substring(this.position, nextIndex).trim();
        ...
        // remaining in the string. For example, this saves â€?textâ€? in â€?This+is++some+textâ€?.
    ...
    private int[] FindNextPhraseLocationAndLength()
    {
    	int firstPhrasePosition = -1;
    	int currentPhrasePosition;
    	int firstPhraseLength = 0;
    	...
    	{
    		currentPhrasePosition = stringToTokenize.indexOf(phrase, position);
    		
    		// Only if we found a phrase and only if is the earliest occurrence 
    		if (currentPhrasePosition > -1 && ((firstPhrasePosition == -1) || (currentPhrasePosition < firstPhrasePosition)))
    		{
    			firstPhrasePosition = currentPhrasePosition;
    			firstPhraseLength = phrase.length();
    		}
    	}
    		    		
    	return new int[] {firstPhrasePosition, firstPhraseLength};
    }
    
    private void LoadPhrases()
    {
    	Phrases = new String[] {"meat free","sugar free", "peanut butter", "ice cream", "gluten free"};
    }
	...
	protected int nextPhraseLength = 0;
